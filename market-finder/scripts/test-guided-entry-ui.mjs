...
test('shows cross-niche drilldown after final results and routes candidates back to eRank', () => {
  assert.match(html, /id="crossNicheSection"/)
  assert.match(html, /id="crossNicheList"/)
  assert.match(html, /id="crossNicheStatus"/)
  assert.ok(position('resultsList') < position('crossNicheSection'))
  assert.ok(position('crossNicheList') < position('buildNextRoundBtn'))
  assert.match(html, /高競合の売れ筋から掘り下げる/)
  assert.match(html, /上位候補を次の調査へ追加/)
  assert.match(app, /buildCrossNicheDrilldown/)
  assert.match(app, /function renderCrossNicheDrilldown\(\)/)
  assert.match(app, /function applyCrossNicheCandidates\(\)/)
  ...
  assert.match(app, /limitNextResearchCandidates\(\[\.\.\.crossNicheCandidates, \.\.\.legacyFollowUps\], 12\)/)
