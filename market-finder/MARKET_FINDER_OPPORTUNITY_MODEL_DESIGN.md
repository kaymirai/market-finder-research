...
### 5.4 大型イベントと通年市場を分離する

ハロウィンやクリスマスの探索では、イベント名、固有モチーフ、固有場面を含む候補を`event-specific`、一般的な相手・職業・趣味へ収束した候補を`evergreen-adjacent`として扱う。最終結果は二つの順位表へ分け、通年候補をイベント固有ニッチとして表示しない。

調査済みの通年候補は、商品語を除いたクラスターキー、調査イベント、確認日時とともに最大500件までローカル保存する。同じ通年クラスターが別イベントで再登場した場合は除外せず、未調査候補の後ろへ送る。この履歴はeRank候補とEtsy公式確認候補の探索順だけに使い、Opportunity点数へ加減算しない。

イベント固有候補が見つからない場合は、通年候補で穴埋めしてイベント市場が有望だったようには見せない。イベント固有欄を0件として残し、「今回は固有候補なし」と明示する。
